...
def comienzo_ahorcado(palabra):
    #Theo Jorge
    """Funcion principal que inicia el juego y llama a las demas funciones"""
    aciertos = 0
    desaciertos = 0
    letras_usadas = ""
    puntaje = 0
    lista_de_letras = [] #lista donde se almacena la palabra letra por letra
    for caracter in palabra: #comando transforma la lista en un signos de pregunta por cada letra
      lista_de_letras += ["?"]
    datos=[palabra,desaciertos,aciertos,letras_usadas,lista_de_letras,puntaje]
    return datos

def ahorcado(palabra,desaciertos,aciertos,letras_usadas,lista_de_letras,puntaje,ultimo):
    desaciertos_viejos=desaciertos
    incognito=presentador(lista_de_letras)
    letra_ingresada=""
    if not ultimo: print("Palabra a adivinar:",incognito,"Aciertos: ",aciertos,"Desaciertos: ",desaciertos,letras_usadas)
    while lista_de_letras.count("?")!=0 and desaciertos != desaciertos_viejos+1 and letra_ingresada != "0" and letra_ingresada != "FIN":
        letra_ingresada = input("Ingrese letra:") #Imput de la letra ingresada
        letra_ingresada = letra_ingresada.lower()
        if validar_letra(letra_ingresada,letras_usadas,lista_de_letras) == True: #validacion de la letra
            desaciertos,aciertos,letras_usadas,incognito,puntaje = acierto_o_desacierto(letra_ingresada,lista_de_letras,palabra,aciertos,desaciertos,letras_usadas,incognito,puntaje)
    veredicto=fin_del_juego(desaciertos,lista_de_letras,letra_ingresada,palabra)
    datos=[palabra,desaciertos,aciertos,letras_usadas,lista_de_letras,puntaje,veredicto]
    return datos
      
    
def presentador(lista): 
    #Theo Jorge
    """Transforma la lista en cadena"""
    palabra = ""
    for i in lista:
        palabra += i
    return palabra
...
def tildar(letra):
    #Theo Jorge
    """Funcion que iguala las vocales con tilde a las mismas sin tilde"""
    if letra == "a": letra = "á"
    if letra == "e": letra = "é"
    if letra == "i": letra = "í"
    if letra == "o": letra = "ó"
...
def validar_letra(letra_ingresada,letras_usadas,lista):
    #Gonzalo Alonso
    """Evalua el caracter ingresado, informando si es una caracter valido, una letra ya ingresada o un comando
    para finalizar el juego"""
    valor = True
    if letra_ingresada == "0" or letra_ingresada == "FIN":# si la lentra ingresada es "0" o "FIN" el juego termina
        valor = False
    elif letras_usadas.count(letra_ingresada) > 0 or lista.count(letra_ingresada)!= 0: # Evalua si la letra ingresada es repetida
        print("Letra ya ingresada")
        valor = False
    elif letra_ingresada.isalpha() == False or len(letra_ingresada) != 1:#Evalua si la lentra ingresada es pertenece al alfabeto
        print("Ingreso invalido")
        valor = False
    return valor
    
def acierto_o_desacierto(letra_ingresada,lista,palabra,aciertos,desaciertos,letras_usadas,incognito,puntaje):
    #Matias Agustin Martinez Saldivar
    """Funcion que evalua si la letra se encuentra o no en la palabra"""
    for caracter in range(0,len(palabra)): #transformar "?" en letras cuando sean acertadas
        if letra_ingresada == palabra[caracter]:
            lista[caracter] = letra_ingresada
        if tildar(letra_ingresada) == palabra[caracter]:
            lista[caracter] = tildar(letra_ingresada)
    incognito=presentador(lista)
    if palabra.count(letra_ingresada) != 0 or palabra.count(tildar(letra_ingresada))!=0: #si la letra esta en la palabra
        aciertos += 1
        print("Muy bien!!! -> Palabra a adivinar:",incognito,"Aciertos:",aciertos,"Desaciertos:",desaciertos,letras_usadas)
        puntaje += 2
    else: #si la letra NO esta en la palabra
        letras_usadas += " - "+letra_ingresada
        desaciertos += 1
        print("Lo siento!!! -> Palabra a adivinar:",incognito,"Aciertos:",aciertos,"Desaciertos:",desaciertos,letras_usadas)
        puntaje -= 1
    return desaciertos,aciertos,letras_usadas,incognito,puntaje

def fin_del_juego(desaciertos,lista_de_letras,letra_ingresada,palabra):
    if desaciertos == 8 or letra_ingresada == "0" or letra_ingresada == "FIN":
        veredicto="Fin del Juego, la palabra era "+palabra+", estás eliminado!!"
    elif lista_de_letras.count("?")==0:
        veredicto="Felicitaciones resolviste el ahorcado con exito"
    else:
        veredicto="Cambio de turno"
    return veredicto

def filtrar_texto(texto):
    #Camilo Aufranc
    """Funcion que filtra separando las palabras que tiene menos de 5 caracteres y se las asigna a un diccionario 
    cuoyo valor es la cantidad de veces que repite en el texto"""
    palabra = ""
    palabras_candidatas = []
    palabras_cantidad = {}
    for i in texto:
        ...
        if not i.isalpha() :
            if len(palabra) >= 5:
                palabras_candidatas+=[palabra.lower()]
            palabra = ""
    for i in palabras_candidatas:
      palabras_cantidad[i] = palabras_candidatas.count(i)
    palabras_cantidad = dict(sorted(palabras_cantidad.items()))
    print(len(palabras_cantidad),"palabras disponibles")
    return palabras_cantidad


def eleccion_palabra(diccionario, longitud=None):
    #Agustin de la Rosa
    """La función deberá devolver aleatoriamente una palabra entre todas las posibles, teniendo en cuenta 
    ...
    if longitud == "":
       longitud = None
    else: longitud=int(longitud)
    for palabras in diccionario.keys():
     if len(palabras) == longitud or longitud == None:
...
def elegir_jugadores():
    jugador="l"
    jugadores=()
    while jugador!="" and len(jugadores)!=5:
     jugador=input("Nombre del jugador:")
     while jugador in jugadores:
         jugador=input("Nombre ya elegido, ingrese otro:")
     if jugador!="":
      jugadores+=jugador,
    return jugadores

def sorteo(ganador,jugadores):
    lista=list(jugadores)
    random.shuffle(lista)
    print("Según el sorteo, el orden de los turnos va a ser este:")
    if ganador!="":
     ubicacion=lista.index(ganador)
     lista[0],lista[ubicacion]=lista[ubicacion],lista[0]
    for i in range(0,len(lista)):
         print(str(i+1)+".",lista[i])
    return lista
         
def asignar_palabras(diccionario,sorteo):
    jugadores={}
    longitud=input("ingrese longitud a jugar:")
    if longitud=="": longitud=random.randint(5,16)
    for jugador in sorteo:
     palabra=eleccion_palabra(diccionario,longitud)
...
texto=obtener_texto()
ahorcado_multijugador_final(texto)
